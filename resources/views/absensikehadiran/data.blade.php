...
<div class="card-body">
<h5 class="card-title">Data Akun Karyawan</h5>
                ...
                <table width="100%" id="example" class="table table-hover table-striped table-bordered" >
				  ...
				  $id="$data->id";?>
                    ...
                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('karyawan.destroy', $data->id) }}" method="POST">
                      <td><a href="{{ route('karyawan.edit', $data->id) }}" title="Edit" ><i class="fa fa-edit fa-w-20"></i></a> ||  <a href="#" title="delete"><button type="submit" class="btn btn-xs"><i class="fa fa-times fa-w-20 text-danger"></i></button></a></td>
                      @csrf
                      @method('DELETE')
                      </form>
                      <td><?php echo $no;?></td>
