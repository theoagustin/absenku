				  ...
				  ?>
                    ...
                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('perusahaan.destroy', $data->id) }}" method="POST">
                     ...
                     <select id="approv" name="approv" class="form-control p-0 konfirm" style="height:30px;">
                      ...
                      <option value="{{ $data->id }}*" <?php if("$data->approv"==""){echo "selected";}?>></option>
                      <option value="{{ $data->id }}*Y" <?php if("$data->approv"=="Y"){echo "selected";}?>>Approve</option>
                      <option value="{{ $data->id }}*N" <?php if("$data->approv"=="N"){echo "selected";}?>>Reject</option>
                    </select>
                      <?php }else{?>
					 <?php if("$data->approv"==""){echo '<div class="alert alert-warning p-0 m-0"> Pending </div>';}elseif("$data->approv"=="N"){echo '<div class="alert alert-danger p-0 m-0"> Rejected </div>';}elseif("$data->approv"=="Y"){echo '<div class="alert alert-success p-0 m-0"> Approve </div>';}?>
                     <?php }?>
